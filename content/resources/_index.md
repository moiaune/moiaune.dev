---
title: "Resources"
date: 2021-02-22T21:29:24+01:00
draft: false
---

This page is a collection of external resources that I found usefull.

## Azure

* [Create an Azure service principal with the Azure CLI](https://docs.microsoft.com/en-us/cli/azure/create-an-azure-service-principal-azure-cli)
* [Naming rules and restrictions for Azure resources](https://docs.microsoft.com/en-us/azure/azure-resource-manager/management/resource-name-rules)
* [Assign Azure roles using the REST API](https://docs.microsoft.com/en-us/azure/role-based-access-control/role-assignments-rest)
* [Azure built-in roles](https://docs.microsoft.com/en-us/azure/role-based-access-control/built-in-roles)

## Go

* [The Go Playground](https://play.golang.org/)
* [Learning Cloud Native Go](https://learning-cloud-native-go.github.io/)
* [Authentication methods in the Azure SDK for Go](https://docs.microsoft.com/en-us/azure/developer/go/azure-sdk-authorization#use-device-token-authentication)

## ServiceNow

* [SN Pro Tips](https://snprotips.com/)
* [ServiceNow JavaScript API Libraries](https://lib.snc.guru)
* [ServiceNow - RESTAPIRequest](https://developer.servicenow.com/dev.do#!/reference/api/quebec/server/sn_ws-namespace/c_RESTAPIRequest)

## Web

* [The Open Graph Protocol](https://ogp.me/)
